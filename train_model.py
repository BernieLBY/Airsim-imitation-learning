...
if os.path.exists(COOKED_DATA_DIR) is False:
    os.mkdir(COOKED_DATA_DIR)
# << The directory in which the model output will be placed >>
MODEL_OUTPUT_DIR = './models/'
if os.path.exists(MODEL_OUTPUT_DIR) is False:
...
train_generator = data_generator.flow\
    (train_dataset['image'], train_dataset['previous_state'], train_dataset['label'], batch_size=batch_size, zero_drop_percentage=0.95, roi=[78,144,1,255]) # [78,144,27,227]
eval_generator = data_generator.flow\
    (eval_dataset['image'], eval_dataset['previous_state'], eval_dataset['label'], batch_size=batch_size, zero_drop_percentage=0.95, roi=[78,144,1,255]) # [78,144,1,255]
...
model = keras.models.Model(inputs=[pic_input], outputs=img_stack)
model.compile(optimizer=adam, loss='mse')

model.summary()

if os.path.exists(os.path.join(MODEL_OUTPUT_DIR,'fresh_models')) is False:
    os.mkdir(os.path.join(MODEL_OUTPUT_DIR,'fresh_models'))
...
history = model.fit_generator(train_generator, steps_per_epoch=num_train_examples//batch_size, epochs=number_of_epochs, callbacks=callbacks,validation_data=eval_generator, validation_steps=num_eval_examples//batch_size, verbose=2)
