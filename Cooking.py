...
def generateDataMapAirSim(folders):
    ...
    for folder in folders:
        ...
        for i in range(1, current_df.shape[0] - 1): 
            ...
            norm_throttle = [ np.clip(float(current_df.iloc[i-1][['Throttle']]),a_min = 0.2, a_max= 0.5) ]
            norm_speed = [ float(current_df.iloc[i-1][['Speed']]) * 3.6 / MAX_SPEED ]  # Normalize speed: between 0 and 1  (kmph)

            # print(norm_throttle, norm_speed)
            ...
            # image_filepath = os.path.join(os.path.join(folder, 'images'), current_df.iloc[i]['ImageFile']).replace('\\', '/')   #ImageName

            tmp_imgfile = current_df.iloc[i]['ImageFile'].split(';')
            
            image_filepath = None
            for imgfile in tmp_imgfile:
                image_filepath = os.path.join(os.path.join(folder, 'images'), imgfile).replace('\\', '/')  # ImageName
                ...
                # print(image_filepath, all_mappings[image_filepath])
...
def cook(folders, output_directory, train_eval_test_split, chunk_size):# chunk_size = batch_size
